...
That is the reason why this idea is not feasible for many players and cards: Because of its large size, the feature vector consumes a lot of memory and requires a lot of additional computations for each training step. Indexing where a card is located from the agent is probably not a solution % TODO really?
...
To find a general state vector for any amount of players, one could, for example, only store the card information % TODO binary or indexed?
