...
Combining the amount of neurons in experiment 6 and the learning rates and update factor in 7, we arrive at the default parameters (except for the feature type which is still 2) which were also used for experiment 9. I consider this experiment the breakthrough because not only did Kraudia achieve a stable win rate over 50~\% (after a few initial hiccups) but also the results did not drop at the end of the experiment suggesting that -- hopefully -- a more static solution has been found. % TODO visual
\medskip
